...
class Viewer {
    ...
    checkViewer(sourceFile, respectOutDir = true) {
        ...
        const url = `http://127.0.0.1:${this.extension.server.port}/viewer.html?file=${encodepath_1.encodePathWithPrefix(pdfFile)}`;
    ...
    getKeyboardEventConfig() {
        ...
        if (setting === 'auto') {
            return os.platform() !== 'linux' || !!((_a = vscode.env.remoteName) === null || _a === void 0 ? void 0 : _a.match(/^(dev-container|attached-container|wsl|ssh-remote)$/));
        }
    ...
    async getPDFViewerContent(pdfFile) {
        ...
        const origUrl = `http://127.0.0.1:${serverPort}/viewer.html?incode=1&file=${encodepath_1.encodePathWithPrefix(pdfFile)}`;
